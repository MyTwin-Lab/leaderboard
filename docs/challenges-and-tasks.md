...
| `completed` | Over. For code (like ML), closing just flips the status — rewards were already paid live, per evaluation run. |
| `archived` | Retired from the listings. Not visible to anonymous visitors. |
...
- **A brief** — the challenge's Markdown introduction, shown before the workspace to a signed-in contributor who hasn't joined (see below)
...
- **GPU compute** — `ml` challenges only, when `compute_enabled` is on and an admin has connected Scaleway: contributors can request a temporary GPU instance — see [`compute-power.md`](./compute-power.md)
