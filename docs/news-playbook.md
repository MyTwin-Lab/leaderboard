...
| `product` | a MyTwin application or feature launches for its users | a roadmap item, a mock-up |
...
|---|---|---|
| 2026-09-16 | — | News are English only, like the whole Lab. Signed Rubens Valcy. `NewsArticle` schema. Categories: partnership, challenge, sandbox, research, community (`product` added 2026-09-26). |
...
| 2026-09-24 | ynov-ai-health-french-response | Category `community`: a talk at an event, 5 February 2026 in Bordeaux. The body is named as Ynov registers it, the *Scientific Council* (Conseil Scientifique), not *committee*. The video is the 15-minute extract of Ynov's recording showing the talk, served by the Lab rather than embedded from YouTube, so readers stay on the page; its poster is a frame of the clip. Rubens's family story is told as he wrote it, without naming the hospital. Ynov is not a partner: its event page is in *Sources*, no link in the text. The list of technologies links to the Lab's own news (mammography, PET scan, Virtuosis). Reworked on 2026-09-25 without the “French response” angle of the original draft: the thread is connecting innovations around the patient; the slug keeps its original wording so published links still work. |
| 2026-09-26 | mytwin-athlete | Category `product` (was `research`: it announces the launch of an app, not a prototype), second chapter of the racket-sports injury-risk story: the lead links back to the August 2025 prototype, which gets an *Update* callout (`NewsCallout` now takes a `label`) and `updatedAt`. The text is the team's, kept as written, including its title and the note on what the 80% does not mean. The illustration is a real screenshot of the app, whose mock-up gauge value is not an article figure: in the cards the frame cuts above it, only the hero shows it. No external source: every figure is internal and labelled as such. The home selection becomes Ynov, Mammography, MyTwin Athlete. |
| 2026-09-26 | ynov-ai-health-french-response | The talk video becomes the illustration: a play button on the cards, the player at the top of the article. The re-exported MP4 had no burned-in subtitles; they are now an English WebVTT track drawn by the Lab's player. |
