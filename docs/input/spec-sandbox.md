...
## 0. Écarts arbitrés avec la spec d'origine

Cette spec est le document de référence fonctionnel, conservé tel qu'il a été validé.
Les points ci-dessous ont été arbitrés **après** sa rédaction, en confrontant la spec à
la maquette `Sandbox Redesign.dc.html` et au code existant. **En cas de contradiction,
c'est cette section qui fait foi.** Le plan d'implémentation détaillé est dans
[plan-sandbox.md](plan-sandbox.md).

| Point | Spec d'origine | Arbitrage retenu |
|---|---|---|
| Description | un champ `description` markdown | **trois champs** : `context`, `goals[]`, `why` — c'est ce que rend la page détail de la maquette |
| Entrées ML | `model_url` et `dataset_urls` tous deux nullable | **dataset requis**, **modèle optionnel** |
| Grille d'évaluation | `code` → grille `code`, `ml` → grille `model` | **grille `code` pour les deux types** : dans un challenge ML, le rôle `model_code` utilise déjà `grid: 'code'`, et le rôle `model` est en `grid: null` (scoré sur métrique, pas sur grille). La grille `model` n'évalue donc jamais de code |
| Type à la promotion | hérité, l'admin ne le choisit pas | **inchangé** — la maquette affichait un sélecteur, il est supprimé |
| Formulaire de promotion | « pool, reward rules, dates, rattachement projet » | **le `CreateChallengeDrawer` existant, pré-rempli**, et non la modale légère de la maquette : un challenge ML a besoin de ses `reward_rules` |
| Promotion ML | non traité | **le travail de l'auteur est repris tel quel** : contributions créées et scoring normal déclenché, qui crédite l'auteur sur le pool. C'est le comportement d'un challenge — y déposer un dataset, un modèle ou du code est une contribution créditée |
| Ledger | deux `rule_key` dans `reward_entries` | **table dédiée `sandbox_rewards`** : `reward_entries.challenge_id` est NOT NULL et tout le leaderboard y agrège par contribution ; un sandbox n'a ni challenge ni contribution et n'a pas à en simuler |
| Visibilité | signed-in uniquement, allowlist anonyme inchangée | **listing et détail publics**, actions réservées aux comptes |
| Stars | signed-in, hors auteur | **stars anonymes autorisées**, et elles **paient les paliers** au même titre que celles des comptes — la newsletter permettra de liker depuis un mail |
| Anti-abus | non traité | **cookie signé** pour l'unicité, **IP hachée** pour le débit seul (30/h) — l'IP ne porte pas l'unicité, sinon un réseau partagé bloquerait de vrais utilisateurs |
| Rôle manager | « admin ou manager » | **admin seul** : un manager est rattaché à un projet, un sandbox n'en a pas. Sur un sandbox, un manager est un contributeur ordinaire |
| Digest | non traité | les **CP sandbox n'entrent pas** dans `cp_distributed`, mais les **nouveaux sandbox y sont affichés** |
| Onglet About | remplacé par Sandbox | **inchangé**, et la route `/about` est conservée, avec un lien depuis le hero de la home |
