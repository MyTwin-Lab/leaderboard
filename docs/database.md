...
| Table | Purpose |
|-------|---------|
| `challenges` | Time-bounded sprints. Columns: `uuid`, `index` (sequential display number), `project_id`, `title`, `slug` (unique, the public URL segment — see [`seo.md`](./seo.md)), `description`, `roadmap`, `status`, `type` (`code`, `ml`, or `validation`, default `code`), `start_date` / `end_date` (both optional), `contribution_points_reward` (the reward pool), `completion`, `reward_rules` (JSON — `ml` *and* `code` challenges, see [`ml-rewards.md`](./ml-rewards.md) and [`challenges-and-tasks.md`](./challenges-and-tasks.md#rewards)), `workspace_mode` (`code` only: `provided_repo` / `own_repo`), `compute_enabled` (`ml` only: activates GPU compute requests, see [`compute-power.md`](./compute-power.md)), `source_challenge_id` (`validation` only — the linked `ml` or `code` challenge, 1:1; its type decides how judging works, see [`validation-challenges.md`](./validation-challenges.md)), `cp_per_validation` and `required_validations` (`validation` only, see [`validation-challenges.md`](./validation-challenges.md)), `created_at`, `closed_at` (stamped when the status reaches `completed`, cleared if it reopens — see [`digest.md`](./digest.md)) |
