...
The schema is divided into domains below. All IDs are UUIDs generated by the database, except where a table is keyed by a declared key (`integration_credentials`, `module_settings`, `cron_runs`, `event_deliveries`).

> **Columns kept until challenge 020 L7.** Several columns were replaced by jsonb or by dedicated tables: `challenges.workspace_mode` / `compute_enabled` / `cp_per_validation` / `required_validations` (→ `flow_config`), the module, digest, sandbox and connection columns of `app_settings` (→ `module_settings`, `integration_credentials`), the five booleans of `onboarding_progress` (→ `onboarding_quest_progress`), and `sandboxes.repo_url` / `model_url` / `dataset_urls` (→ `proposal_fields`). `db:apply-schema` copied them over; the application no longer reads them, and they are dropped after deployment is verified (see [`deployment.md`](./deployment.md)).

### Users & Auth

| Table | Purpose |
|-------|---------|
| `users` | Contributors. Key columns: `uuid`, `email`, `full_name`, `google_user_id` (linked Google account), `github_username`, `bio`, `role`, `avatar_url`, `created_at`. `role` is free-text in the schema; the values the app assigns are `admin`, `contributor` and `viewer` (see [`auth.md`](./auth.md#roles)). Former `medical_pro` accounts became `contributor` holding the `medical_pro` qualification. There is no "manager" role — see below. |
| `refresh_tokens` | Stores hashed refresh tokens linked to a user. Used for JWT token rotation. |
| `user_qualifications` | Qualifications granted to a user by an admin: `user_id`, `key` (declared by the distribution, e.g. `medical_pro`), `granted_by`, `granted_at`, `note`. One row per (user, key). A flow's `flow_config` names the qualification its gestures require. |
| `qualification_changes` | Append-only audit of grants and revocations: `user_id`, `key`, `action` (`granted` / `revoked`), `changed_by`, `note`, `created_at`. Written in the same transaction as the change. |
| `onboarding_quest_progress` | One row per completed quest: `user_id`, `quest_key`, `completed_at`. Quests are declared by their owners (flows, modules) and recorded from platform events. See [`onboarding.md`](./onboarding.md). |
| `onboarding_progress` | Legacy: one row per user with five boolean quest flags. No longer read; dropped in L7. |
...
| Table | Purpose |
|-------|---------|
| `challenges` | Time-bounded sprints. Columns: `uuid`, `index` (sequential display number), `project_id`, `title`, `slug` (unique, the public URL segment — see [`seo.md`](./seo.md)), `description`, `roadmap`, `status`, `type` (the key of an installed flow: `code`, `ml`, `endpoint-validation`, `journey-validation` or `data-annotation`), `start_date` / `end_date` (both optional), `contribution_points_reward` (the reward pool), `completion`, `reward_rules` (JSON, parsed by the flow — see [`ml-rewards.md`](./ml-rewards.md) and [`challenges-and-tasks.md`](./challenges-and-tasks.md#rewards)), `flow_config` (jsonb validated by the flow's schema: `workspace_mode` for `code`, the compute extension's `compute_enabled` for `ml`, `cp_per_validation`, `required_validations`, `reviewer_qualification`, `eligible_roles` and `expert_comment_qualification` for validation flows — see [`writing-a-flow.md`](./writing-a-flow.md)), `flow_config_version` (upgraded by `db:upgrade-flow-configs`), `source_challenge_id` (a generic link to a parent challenge — the challenge a validation flow checks; a partial unique index on (`source_challenge_id`, `type`) allows one of each validation flow per source, see [`validation-challenges.md`](./validation-challenges.md)), `created_at`, `closed_at` (stamped when the status reaches `completed`, cleared if it reopens — see [`digest.md`](./digest.md)) |
...
| `challenge_slug_redirects` | A challenge's former slugs: `slug` (PK), `challenge_id` (cascade). A page URL under one of them answers a 308 to the current slug. A slug is never both current for one row and a redirect to another — `ChallengeRepository.isSlugTaken` reads both tables. |
...
| Table | Purpose |
|-------|---------|
| `contributions` | A recorded unit of work. Columns: `uuid`, `challenge_id`, `user_id`, `task_id` (null for ML contributions), `type`, `title`, `description`, `tags` (JSON string array), `evaluation` (JSON), `evaluation_status` (`pending` / `running` / `done` / `failed` / `skipped_reuse`), `artifact_url` (normalized submission URL, used to detect reuse between contributors on ML challenges), `live_endpoint_url` (the deployed URL of whatever contribution is exposed as a validation target — `api_packaging` in reference-case mode, `project` in scenario mode; distinct from `artifact_url`, and typed by an admin at exposure time, never declared by the contributor — see [`validation-challenges.md`](./validation-challenges.md)), `reward`, `submitted_at` (**last** submission — rewritten on every re-evaluation of a code challenge, and used by `lineage.ts` for reuse seniority), `created_at` (actual creation date, added for [`digest.md`](./digest.md)) |
...
| `validation_targets` | Which contribution is exposed on a validation challenge (`endpoint-validation` / `journey-validation`) — `api_packaging` in reference-case mode, `project` in scenario mode. Columns: `uuid`, `validation_challenge_id`, `contribution_id`, `position`, `outcome` (`pending` → permanently `works` / `broken` once quorum resolves it — **reference-case mode only**; `resolve()` is called only from `castVerdict`, never from the walkthrough service, so a scenario-mode target stays `pending` forever by design), `resolved_at`. Unique on (challenge, contribution). |
| `validation_reference_cases` | Ground-truth cases authored by a reviewer holding the challenge's `reviewer_qualification` (`medical_pro` in MyTwin): `input_bytes` / `input_filename` / `input_content_type` and `expected_output_bytes` / `expected_output_filename` / `expected_output_content_type` (`bytea`), plus `author_user_id`. Exactly `required_validations` cases per challenge, enforced in `ReferenceCaseService`. The expected output is only ever SELECTed by `ReferenceCaseRepository.findExpectedOutputById` — the single enforcement point for "never leaked before reveal". |
...
| `evaluation_runs` | A record of one `evaluate()` call (trigger type — the key of the flow or extension that asked —, optional time window, status, error info, subject in `meta` when no contribution is linked, as for sandboxes). Viewable and retriable by admins — see [`evaluation.md`](./evaluation.md). |
...
| Table | Purpose |
|-------|---------|
| `app_settings` | Singleton row (`id = 1`) holding the instance-wide theme (`theme_key`, optional custom `primary_color` / `background_color`, `theme_mode`). Its former module, digest, sandbox and connection columns are no longer read (dropped in L7). See [`admin-settings.md`](./admin-settings.md). |
| `module_settings` | One row per product module (`key`): `enabled`, `settings` (jsonb validated by the module's schema — e.g. the digest's `frequency_days`, the sandbox's `star_tiers` and `promotion_bonus_cp`), `updated_at`, `updated_by`. A module without a row takes the default it declares. Read through the `modules` capability (`packages/capabilities/modules.ts`). |
| `integration_credentials` | One row per connected integration (`key`: `github`, `kaggle`, `slack`, `openai`, `scaleway`): the AES-encrypted secret (`secret_enc`, `secret_iv`), `meta` (what can be read without the secret — organisation, workspace, project…), `connected_at`, `connected_by`. Read through the `credentials` capability. |

### Resources

Claimable work units of the core `resources` capability (`packages/capabilities/resources.ts`). The flow names the types and owns the meaning of payloads, results and verdicts — see [`data-annotation.md`](./data-annotation.md).

| Table | Purpose |
|-------|---------|
| `resource_instances` | One unit of work: `uuid`, `challenge_id` (cascade), `resource_type` (`item`, `gold`…), `payload` (jsonb, validated by the flow), `class` (optional bucket filtered at draw time), `state` (`open` / `closed`), `verdict`, `resolution` (jsonb), `created_by`, `created_at`, `closed_at`. Indexed on (`challenge_id`, `resource_type`, `state`, `class`). |
| `resource_claims` | One claim = one unit of work = one result: `uuid`, `resource_id` (cascade), `challenge_id` (denormalized), `user_id`, `result` (jsonb, set at consumption), `claimed_at`, `expires_at`, `consumed_at`, `released_at`. Active = none of consumed / released / expired. The partial unique index on (`resource_id`, `user_id`) `WHERE released_at IS NULL` keeps one live claim per person per resource. |

### Platform runtime

| Table | Purpose |
|-------|---------|
| `cron_runs` | One row per scheduled job (`job_key`): `last_started_at`, `last_finished_at`, `last_status` (`running` / `succeeded` / `failed`), `last_error`, and `locked_until` — the lock that keeps a job from running twice. Written by `/api/cron/tick` (`packages/capabilities/cron.ts`). |
| `platform_events` | The event outbox: `id` (serial, the delivery order), `type`, `payload` (jsonb), `occurred_at`. An event is written only if something subscribes to its type, inside the emitting transaction when there is one. Purged after 30 days by `core.events.purge`. |
| `event_deliveries` | One cursor per subscriber (`subscriber_key`): `last_event_id` delivered, `last_error`, `updated_at`. `core.events.distribute` delivers in order and stops a subscriber's queue on its first error. |
...
| Table | Purpose |
|-------|---------|
| `sandboxes` | A contributor-proposed unit of work, outside the challenge system: no pool, no members, no tasks. `slug` is unique among sandboxes (its own namespace, separate from challenges'), with former slugs in `sandbox_slug_redirects` like challenges'. `type` (the key of a flow that declares `proposable`, `varchar(64)`) is immutable, the proposal is held by `context` / `goals` / `why` and by `proposal_fields` (jsonb validated by the flow's schema — `repo_url`, `model_url`, `dataset_urls`; the old columns are still written in mirror until L7), and the formative evaluation lives on the row (`evaluation`, `evaluation_status`, `evaluated_at`) because it pays no CP and must not reach the ledger. `promoted_challenge_id` is `ON DELETE SET NULL` — deleting the challenge must not erase the proposal behind it. |
...
challenges ──< challenge_slug_redirects
...
challenges ─── challenges (source_challenge_id, validation → ml or code, 1:1)
...
users ──< onboarding_quest_progress
users ──< user_qualifications
users ──< qualification_changes
users ──< refresh_tokens

digests (standalone — no FK; the payload is denormalized on purpose)
platform_events, event_deliveries, cron_runs (standalone — keyed by type, subscriber and job)
```
...
- `db_data/seed-annotation.ts` (`npm run db:seed:annotation`) — a demo annotation campaign labeled through the real flow actions
