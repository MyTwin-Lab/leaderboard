...
The schema is divided into eight domains. All IDs are UUIDs generated by the database.
...
| Table | Purpose |
|-------|---------|
| `app_settings` | Singleton row (`id = 1`) holding instance-wide admin settings: active theme (`theme_key`, optional custom `primary_color` / `background_color`, `theme_mode`), module visibility toggles (`modules_meetings_enabled`, `modules_onboarding_enabled`), and the encrypted GitHub/Kaggle/Slack/OpenAI/Scaleway connection state. See [`admin-settings.md`](./admin-settings.md). |
