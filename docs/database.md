...
| Table | Purpose |
|-------|---------|
| `challenges` | Time-bounded sprints. Columns: `uuid`, `index` (sequential display number), `project_id`, `title`, `slug` (unique, the public URL segment — see [`seo.md`](./seo.md)), `description`, `roadmap`, `status`, `type` (the key of an installed flow: `code`, `ml`, `endpoint-validation` or `journey-validation`), `start_date` / `end_date` (both optional), `contribution_points_reward` (the reward pool), `completion`, `reward_rules` (JSON, parsed by the flow — see [`ml-rewards.md`](./ml-rewards.md) and [`challenges-and-tasks.md`](./challenges-and-tasks.md#rewards)), `flow_config` (jsonb validated by the flow's schema: `workspace_mode` for `code`, the compute extension's `compute_enabled` for `ml`, `cp_per_validation`, `required_validations`, `reviewer_qualification`, `eligible_roles` and `expert_comment_qualification` for validation flows — see [`writing-a-flow.md`](./writing-a-flow.md)), `flow_config_version` (upgraded by `db:upgrade-flow-configs`), `source_challenge_id` (a generic link to a parent challenge — the challenge a validation flow checks; a partial unique index on (`source_challenge_id`, `type`) allows one of each validation flow per source, see [`validation-challenges.md`](./validation-challenges.md)), `created_at`, `closed_at` (stamped when the status reaches `completed`, cleared if it reopens — see [`digest.md`](./digest.md)) |
