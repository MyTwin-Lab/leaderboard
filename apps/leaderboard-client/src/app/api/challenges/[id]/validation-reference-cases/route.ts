...
// admin/manager: every case on the challenge (oversight). qualified reviewer: only
...
// POST /api/challenges/[id]/validation-reference-cases — qualified reviewer only.
// No admin/manager override — per SPEC 3, only a qualified reviewer writes cases.
...
export async function POST(
  ...
  try {
    ...
    const { id: challengeId } = await params;
    if (!(await isQualifiedReviewer(user.id, challengeId))) {
      return NextResponse.json({ error: 'Only qualified reviewers can author a reference case' }, { status: 403 });
    }
