 ...
 * Lab, et c'est ce moment qui intéresse le lecteur.
 */
export function NewsCard({
  ...
  article: NewsArticle;
  titleAs?: "h2" | "h3";
  /** La une de `/news` : même carte, un cran de typographie au-dessus. */
  featured?: boolean;
  className?: string;
}) {
  return (
    <Link
      ...
      className={cn(
        "animate-fade-up group flex h-full min-w-0 flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-[0_14px_40px_-26px_rgba(0,0,0,0.6)] transition-all duration-300 hover:-translate-y-0.5 hover:border-brandCP/25 hover:bg-white/[0.06] sm:p-6",
        featured && "sm:p-8",
        className,
      )}
    >
