...
describe("fetchHomeOverview", () => {
  ...
  it.each(["archived", "completed"])(
    "keeps %s challenges out of trending even when they saw recent activity",
    async (closedStatus) => {
      // Recent activity on a closed challenge is exactly when it would surface,
      // and it is the one time it must not: the fallback path already states that
      // drafts and closed stay hidden, so both paths have to agree. 'completed'
      // et 'archived' ferment tous deux le join (joinGate.isOpen) — mettre en
      // avant un challenge qu'on ne peut plus rejoindre n'a pas de sens.
      vi.useFakeTimers();
      vi.setSystemTime(NOW);
      ...
      vi.spyOn(repositories.challenge, "findAll").mockResolvedValue([
        ...
        { uuid: "c2", title: "Retired", status: closedStatus, type: "code", index: 2, contribution_points_reward: 100, completion: 0, project_id: "p1", start_date: new Date(), end_date: new Date() },
      ] as any);
      vi.spyOn(repositories.user, "findAll").mockResolvedValue([] as any);
      vi.spyOn(repositories.contribution, "findAll").mockResolvedValue([
        // The closed one is busier, so ranking alone would put it first.
